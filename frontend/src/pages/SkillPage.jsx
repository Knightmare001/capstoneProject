...
export default function SkillPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (selectedSkills.length === 0) {
      setError("Pilih minimal 1 skill yang kamu kuasai.");
      return;
    }
    if (!minSalary || !maxSalary) {
      setError("Isi ekspektasi gaji minimum dan maksimum.");
      return;
    }
      ...
      <div className="max-w-7xl mx-auto w-full px-6 lg:px-16 py-12 flex flex-col lg:flex-row gap-12 lg:gap-20 relative z-10 flex-1">
        ...
        <div className="w-full lg:w-5/12 flex flex-col lg:sticky lg:top-32 h-fit">
          ...
          <p className="text-lg text-text-main/70 mb-10 leading-relaxed font-medium">
            Pilih skill yang sudah kamu miliki, masukkan ekspektasi gaji, dan kami akan mencocokkan dengan data job
            market untuk merekomendasikan skill yang paling perlu kamu tingkatkan.
          </p>
          ...
          <div className="flex flex-col gap-3">
            {[
              ...
              <div
                key={i}
                className="flex items-start gap-3 p-4 bg-white/70 rounded-2xl border border-secondary/15 shadow-sm"
              >
                <div>
