...
const inputClass =
  "w-full bg-background border-2 border-secondary/20 rounded-2xl p-3.5 text-text-main font-medium focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/50 transition-all placeholder:text-text-main/30 placeholder:font-normal shadow-sm";
      ...
      <div className="max-w-7xl mx-auto w-full px-6 lg:px-16 py-12 flex flex-col lg:flex-row gap-12 lg:gap-20 relative z-10 flex-1">
                ...
                return (
                  <div key={s.num} className="relative flex items-center gap-4">
                    {index !== STEPS.length - 1 && (
                      <div
                        className={`absolute left-5 top-10 w-[2px] h-6 -ml-px ${isCompleted ? "bg-primary" : "bg-secondary/20"}`}
                      ></div>
                    )}
                    <div
                    ...
                    <div
                      ...
                      className={isCompleted ? "cursor-pointer hover:opacity-80" : ""}
                    >
                      <p
                        className={`font-bold transition-colors ${isActive ? "text-text-main" : isCompleted ? "text-primary" : "text-text-main/40"}`}
                      >
                        {s.label}
                      </p>
                      <p
                        className={`text-xs font-medium ${isActive || isCompleted ? "text-text-main/60" : "text-text-main/30"}`}
                      >
                        {s.desc}
                      </p>
                    </div>
        ...
        <div className="w-full lg:w-7/12">
          <form
            ...
            className="bg-white/80 backdrop-blur-md rounded-[40px] p-6 sm:p-10 shadow-2xl border border-secondary/20 flex flex-col gap-6 relative"
          >
            {/* TAHAP 1 */}
            {step === 1 && (
            ...
            {step === 2 && (
              <div className="flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-8 duration-500">
                <SectionCard title="Skala Kepuasan">
                  <p className="text-xs text-text-main/50 font-medium mb-1">
                    Skala 1 (Sangat Rendah) — 4 (Sangat Tinggi)
                  </p>
                  <RatingInput
                    label="Kepuasan Pekerjaan"
                ...
                <SectionCard title="Beban Kerja">
                  <ToggleInput
                    label="Sering Lembur (OverTime)"
                    name="overTime"
                    value={form.overTime}
                    onChange={setField}
                    description="Apakah kamu dituntut bekerja di luar jam operasional?"
                  />
                </SectionCard>
              ...
              <div className="flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-8 duration-500">
                <SectionCard title="Finansial (IDR)" icon={<FiDollarSign />}>
                  <div className="flex flex-wrap gap-5 items-start">
                    <div className={flexFieldClass}>
                      ...
                      {form.monthlyIncome && (
                        <p className="text-xs text-primary font-bold pl-1 animate-in fade-in duration-300">
                          {formatRupiah(form.monthlyIncome)} / bulan
                        </p>
                      )}
                    </div>
                    <div className={flexFieldClass}>
                      <Field
                        ...
                        description="Biaya hidup tiap bulan (makan, transport, dll)"
                      >
                        <input
                          type="number"
                      ...
                      {form.monthlyExpenses && (
                        <p className="text-xs text-primary font-bold pl-1 animate-in fade-in duration-300">
                          {formatRupiah(form.monthlyExpenses)} / bulan
                        </p>
                      )}
                    </div>
                    <div className={flexFieldClass}>
                      <Field
                        ...
                        description="Total tagihan yang ada perbulannya (PayLater, pinjol, dll)"
                      >
                        <input
                          type="number"
                      ...
                      {form.monthlyDebtObligations && (
                        <p className="text-xs text-primary font-bold pl-1 animate-in fade-in duration-300">
                          {formatRupiah(form.monthlyDebtObligations)} / bulan
                        </p>
                      )}
                    </div>
                    <div className={flexFieldClass}>
                      ...
                      {form.monthlySavings && (
                        <p className="text-xs text-primary font-bold pl-1 animate-in fade-in duration-300">
                          {formatRupiah(form.monthlySavings)} / bulan
                        </p>
                      )}
                    </div>
                  </div>
                ...
                <SectionCard title="Kesiapan Transisi">
                  ...
                  <div className="flex flex-col gap-3 mt-4">
                    <ToggleInput
                      label="Memiliki Tanggungan?"
                      name="hasDependents"
                      value={form.hasDependents}
                      onChange={setField}
                      description="Istri, anak, atau orang tua yang dibiayai."
                    />
                    <ToggleInput
                      label="Asuransi Kesehatan Pribadi"
                      name="hasHealthInsurance"
                      value={form.hasHealthInsurance}
                      onChange={setField}
                      description="Punya BPJS Mandiri atau asuransi swasta jika *resign*."
                    />
                    <ToggleInput
                      label="Punya Side Hustle / Freelance?"
                      name="hasSideHustle"
                      value={form.hasSideHustle}
                      onChange={setField}
                      description="Ada pemasukan sampingan untuk bantalan sementara."
                    />
                  </div>
                </SectionCard>
                {errorMsg && (
                ...
                <div className="flex gap-4">
                  ...
                  >
                    {loading ? (
                      <>
                        <FiLoader className="animate-spin" /> Sedang Menghitung...
                      </>
                    ) : (
